...
sub print_standard_usage
{
  ...
  print "Usage:  
perl $plname [options] [testname...] 

options are: 
  --detail   -- Detailed decription of a test is given before it is run.
                Note: Not all tests have descriptions.
  --debug    -- Show all system calls issued and the return status.
                You might use this if a test fails.
  --keep     -- Do not remove any of the files generated by the tests.
                Normally, files generated by tests are remove if the test
                passes.
  --profile
  --usage
  --help                 -- This help
...
is stripped as well as any portion of a test named which ends \"scripts/\".
...
sub parse_command_line
{
  ...
  # located) and dropping trailing .pl (which is the extension on
  # test names).
