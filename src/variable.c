...
{
  variable_t const *key = (variable_t const *) keyv;
...
{
  variable_t const *x = (variable_t const *) xv;
  variable_t const *y = (variable_t const *) yv;
                        ...
                        const struct floc *flocp)
{
  ...
  var_slot = (variable_t **) hash_find_slot (&set->table, &var_key);
  ...
  v = (variable_t *) xmalloc (sizeof (struct variable));
...
static variable_t *
handle_special_var (variable_t *var)
{
  ...
  */
  ...
  if (streq (var->name, ".VARIABLES")
      && global_variable_set.table.ht_fill != last_var_count)
    {
      ...
      unsigned long len;
      char *p;
      variable_t **vp = (variable_t **) global_variable_set.table.ht_vec;
      variable_t **end = &vp[global_variable_set.table.ht_size];
                     ...
                     variable_set_t *from_set)
{
  ...
  for ( ; from_var_slot < from_var_end; from_var_slot++)
    if (! HASH_VACANT (*from_var_slot))
      {
	variable_t *from_var = *from_var_slot;
	variable_t **to_var_slot
	  = (variable_t **) hash_find_slot (&to_set->table, *from_var_slot);
...
define_automatic_variables (void)
{
     ...
     and use $COMSPEC as the default $SHELL when appropriate.  */
  {
    ...
    variable_t *mshp = lookup_variable ("MAKESHELL", 9);
    variable_t *comp = lookup_variable ("COMSPEC", 7);
	...
	variable_t *shp = lookup_variable (shell_str, shlen);
...
target_environment (struct file *file)
{
	...
	if (! HASH_VACANT (*v_slot))
	  {
	    ...
	    new_slot = (variable_t **) hash_find_slot (&table, v);
  ...
  v_slot = (variable_t **) table.ht_vec;
                        ...
                        enum variable_flavor flavor, int target_var)
{
  ...
  if ((origin == o_file || origin == o_override)
      ...
      else
	{
	  ...
	  variable_t *pathv = lookup_variable ("PATH", 4);
	  char *path_string;
	  char *fake_env[2];
	  size_t pathlen = 0;
...
variable_t *
parse_variable_definition (variable_t *v, char *line)
{
  int c;
  char *p = line;
  char *beg;
    ...
    for (p = pattern_vars; p != 0; p = p->next)
      {
        ++rules;
        printf ("\n%s :\n", p->target);
        print_variable_info (&p->variable, "# ");
      }
