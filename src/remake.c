/* $Id: remake.c,v 1.14 2005/12/07 01:37:18 rockyb Exp $
...
static int
update_file_1 (file_t *file, unsigned int depth, 
	       target_stack_node_t *p_call_stack)
{
  ...
  if (noexist) {
    ...
    if ( debugger_stepping || debugger_nexting ) 
  ...
  if ( debugger_enabled && debugger_stepping ) 
...
void
notice_finished_file (file_t *file)
{
  dep_t *d;
...
f_mtime (file_t *file, int search)
{
  ...
  if (ar_name (file->name))
      ...
      if (search && strcmp (arfile->hname, arname))
	{
	  ...
	  memmove (name, arname, arlen);
	  name[arlen] = '(';
	  memmove (name + arlen + 1, memname, memlen);
	  name[arlen + 1 + memlen] = ')';
