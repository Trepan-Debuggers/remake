/* $Id: job.c,v 1.28 2005/12/18 18:24:21 rockyb Exp $
			       ...
			       target_stack_node_t *p_call_stack)
{
  ...
  /* If we are executing a target pattern, we want the location
     information for the target pattern, not where the target is
     defined. Below we'll just unconditionally use the location
     information from the child in the call stack. We can change
     values like this in the call stack since it has its own allocated
     copy. FIXME: we might want to record both the target pattern name
     and the target that matches this patter. Right now we save
     the target name and not the pattern name.
   */
  ...
  flags = (p_child->file->command_flags
  ...
  if (argv == 0)
    ...
    next_command:
      ...
      if (job_next_command (p_child))
	start_job_command (p_child, p_call_stack);
      else
	{
	  /* No more commands.  Make sure we're "running"; we might not be if
             (e.g.) all commands were skipped due to -n.  */
          set_command_state (p_child->file, cs_running);
	  p_child->file->update_status = 0;
	  notice_finished_file (p_child->file);
	}
      return;
    }

  if ( skip_execution == print_child_cmd(p_child, p_call_stack) )
    goto next_command;
  ...
  */
  ...
  if (just_print_flag || tracing || i_debugger_stepping || i_debugger_nexting
  ...
  p_child->good_stdin = !good_stdin_used;
  ...
  if (p_child->remote)
    {
      ...
      if (start_remote_job (argv, p_child->environment,
			    p_child->good_stdin ? 0 : bad_stdin,
	...
	{
	  if (p_child->good_stdin && !used_stdin)
	    {
	      p_child->good_stdin = 0;
	      good_stdin_used = 0;
	    }
	  p_child->remote = is_remote;
	  p_child->pid = id;
	}
    }
  else
      ...
      if (p_child->pid == 0)
	{
	  ...
	  child_execute_job (p_child->good_stdin ? 0 : bad_stdin, 1,
                             argv, p_child->environment);
	}
      else if (p_child->pid < 0)
	{
	  /* Fork failed!  */
	  unblock_sigs ();
...
#ifdef __MSDOS__
  {
	...
	execute_by_shell = 0;	/* for the next time */
      ...
      {
	dos_command_running = 1;
	proc_return = spawnvpe (P_WAIT, argv[0], argv, p_child->environment);
      }
...
#ifdef WINDOWS32
  {
      ...
      hPID = process_easy(argv, p_child->environment);

      if (hPID != INVALID_HANDLE_VALUE)
        p_child->pid = (int) hPID;
      else {
        int i;
        unblock_sigs();
        fprintf(stderr,
...
int
child_execute_job (int stdin_fd, int stdout_fd, 
		   char **argv, char **envp, target_stack_node_t *p_call_stack)
{
  int pid;
